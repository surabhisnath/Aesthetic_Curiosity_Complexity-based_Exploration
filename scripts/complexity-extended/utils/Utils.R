...
modelanalysis <- function(num_folds, models, data_train_folds, data_test_folds, to_print, to_plot, fullformula) {
    ...
    rsqtrains[x] <- cor(predict(models[[x]], data_train_folds[[x]]), data_train_folds[[x]]$visible_complexity_rating)^2
    rsqtests[x] <- cor(predict(models[[x]], data_test_folds[[x]]), data_test_folds[[x]]$visible_complexity_rating)^2
    RMSEtrains[x] <- sqrt(mean(residuals(models[[x]])^2))
    RMSEtests[x] <- sqrt(mean((predict(models[[x]], data_test_folds[[x]]) - data_test_folds[[x]]$visible_complexity_rating)^2))
    }
...
make_plots <- function(model, data_train_fold, data_test_fold) {
    # Make and save plots
    # Save plot of visible_complexity_rating vs predictions (on both train and test data)

    pdf(file = "plots/complexity-extended_train.pdf", width = 10, height = 10, family = "Times")
    par(mar = c(5, 6, 4, 1) + .1)
    p1 <- plot(predict(model, data_train_fold), data_train_fold$visible_complexity_rating, xlab = "Predictions on training data", ylab = "Number of Clicks", cex.axis = 3, cex.lab = 3, col = rgb(red = 0, green = 0, blue = 0, alpha = 0.3), pch = 16, axes = FALSE)
    axis(1, cex.axis = 2)
    axis(2, cex.axis = 2)
    p1 + theme(
    ...
    pdf(file = "plots/complexity-extended_test.pdf", width = 10, height = 10, family = "Times")
    par(mar = c(5, 6, 4, 1) + .1)
    p2 <- plot(predict(model, data_test_fold), data_test_fold$visible_complexity_rating, xlab = "Predictions on test data", ylab = "Number of Clicks", cex.axis = 3, cex.lab = 3, col = rgb(red = 0, green = 0, blue = 0, alpha = 0.3), pch = 16, axes = FALSE)
    axis(1, cex.axis = 2)
    axis(2, cex.axis = 2)
    p2 + theme(
        plot.title = element_text(family = "serif", size = 16),
