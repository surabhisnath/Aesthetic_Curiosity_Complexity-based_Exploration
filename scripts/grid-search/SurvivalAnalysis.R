...
{
  selected_data <-
    data[, c("uLSC", "vLSC", "uInt", "vInt", "uLSCsq",
             "vLSCsq", "uIntsq", "vIntsq")]
  correlation_matrix <- cor(selected_data)
  colnames(correlation_matrix) <-
    c("uLSC", "vLSC", "uInt", "vInt", "uLSC^2",
      "vLSC^2", "uInt^2", "vInt^2") # Rename columns
  ...
  axis(1,
       at = 1:ncol(correlation_matrix),
       labels = colnames(correlation_matrix),
       las = 2, cex.axis = 1.4,
       family = "serif")
  axis(2,
       at = 1:nrow(correlation_matrix),
...
models <- list(
  ...
  "uLSCsq + uIntsq + vLSCsq + vIntsq + uLSC:vLSC + uInt:vInt + Subject",
  "(uLSCsq + vLSCsq + uLSC:vLSC) + (uIntsq + vIntsq + uInt:vInt) + (uLSC + vLSC):(uInt + vInt) + Subject",
  "uLSC + vLSC + uInt + vInt + uLSCsq + uIntsq + vLSCsq + vIntsq + Subject",
  "uLSC + vLSC + uInt + vInt + uLSCsq + uIntsq + vLSCsq + vIntsq + uLSC:vLSC + uInt:vInt + Subject",
  "uLSC + vLSC + uInt + vInt + uLSCsq + uIntsq + vLSCsq + vIntsq + uLSC:vLSC + uInt:vInt + uLSC:uInt + vLSC:vInt + Subject",
  "uLSC + vLSC + uInt + vInt + uLSCsq + uIntsq + vLSCsq + vIntsq + uLSC:vLSC + uInt:vInt + uLSC:uInt + vLSC:vInt + uLSC:vInt + uInt:vLSC + Subject"
)

# Save all results to model_fits/Table_1_click_level.csv
{
  df <-
    data.frame(matrix(ncol = 8,
                      nrow = 0,
                      dimnames = list(NULL,
                                      c("Id", "model", "AIC", "BIC",
                                        "Concordance train", "Concordance test",
                                        "log rank p", "likelihood ratio p"))))
  ...
  for (formula in models) {
    ...
    for (x in 1:num_folds) {
      ...
      pred_train <-
        predict(model, newdata = data_train_folds[[x]], type = "risk")
    ...
    df[nrow(df) + 1, ] <-
      c(id, noquote(fullformula),
        c(mean(AICs), mean(BICs), mean(concordances_train),
          mean(concordances_test), max(logrankp), max(likratiop)))
  }
...
{
  bestformula <-
    as.formula("Surv(click_id, move_on) ~ uLSC * vLSC + uInt * vInt + Subject")
  ...
  combined_data <- data.frame(Variable = coef_names,
                              Coefficients = coefficients,
                              ExpCoefficients = exp_coefficients,
                              Significance = signif_levels)

  # Write the data frame to a CSV file
  write.csv(combined_data,
            file = "model_fits/Table_2_coeff_move_on.csv",
            row.names = FALSE)
}

# Plot survival curves keeping all but one or two
#  (for interactions) variables constant - Figure 2
{
  # vLSC manipulation - Figure 2a
  newdata <- data.frame(
    vLSC =
      seq(mean(data$vLSC, na.rm = TRUE) - 2 * sd(data$vLSC, na.rm = TRUE),
          mean(data$vLSC, na.rm = TRUE) + 2 * sd(data$vLSC, na.rm = TRUE),
          length.out = 3),
  ...
  newdata <- data.frame(
    uInt =
      seq(mean(data$uInt, na.rm = TRUE) - 2 * sd(data$uInt, na.rm = TRUE),
          mean(data$uInt, na.rm = TRUE) + 2 * sd(data$uInt, na.rm = TRUE),
          length.out = 3),
    uLSC = mean(data$uLSC, na.rm = TRUE),
  ...
  newdata <- data.frame(
    vInt =
      seq(mean(data$vInt, na.rm = TRUE) - 2 * sd(data$vInt, na.rm = TRUE),
          mean(data$vInt, na.rm = TRUE) + 2 * sd(data$vInt, na.rm = TRUE),
          length.out = 3),
    uLSC = mean(data$uLSC, na.rm = TRUE),
    vLSC = mean(data$vLSC, na.rm = TRUE),
    uInt = mean(data$uInt, na.rm = TRUE),
  ...
  v1 <-
    seq(mean(data$uLSC, na.rm = TRUE) - 2 * sd(data$uLSC, na.rm = TRUE),
        mean(data$uLSC, na.rm = TRUE) + 2 * sd(data$uLSC, na.rm = TRUE),
        length.out = 2)  # Values of uLSC for the three scenarios
  v2 <-
    seq(mean(data$vLSC, na.rm = TRUE) - 2 * sd(data$vLSC, na.rm = TRUE),
        mean(data$vLSC, na.rm = TRUE) + 2 * sd(data$vLSC, na.rm = TRUE),
        length.out = 2)   # Values of vLSC for the three scenarios
  newdata <- expand.grid(
    uLSC = v1,
    vLSC = v2
  )
