  ...
  function(re, QQ = FALSE, likeDotplot = TRUE, detailedFacetLabs = TRUE) {
    f <- function(x, nm = "ranef plot") {
      ...
      p <- p + geom_hline(yintercept = 0,
                          lwd = I(7 / 12),
                          colour =
                            I(grDevices::hsv(0 / 12, 7 / 12, 7 / 12)),
                          alpha = I(5 / 12))
      p <- p +
        geom_errorbar(aes_string(ymin = "y - ci",
                                 ymax = "y + ci"),
  ...
  function(num_folds, models, data_train_folds,
    ...
    for (x in 1:num_folds) {
      ...
      rsqtrains[x] <- cor(predict(models[[x]],
                                  data_train_folds[[x]]),
                          data_train_folds[[x]]$num_clicks)^2
...
make_plots <- function(model, data_train_fold, data_test_fold) {
  ...
  pdf(file = "plots/num_clicks_train.pdf",
      width = 10, height = 10, family = "Times")
  par(mar = c(5, 6, 4, 1) + .1)
  p1 <- plot(predict(model, data_train_fold),
             data_train_fold$num_clicks,
             xlab = "Predictions on training data",
             ylab = "Number of Clicks",
             cex.axis = 3, cex.lab = 3,
             col = rgb(red = 0, green = 0, blue = 0, alpha = 0.3),
             pch = 16, cex = 1.5, axes = FALSE)
  x_range <- range(predict(model, data_train_fold))
  axis(1, at = seq(from = floor(x_range[1] / 20) * 20,
                   to = ceiling(x_range[2] / 20) * 20,
                   by = 20), cex.axis = 2)
  # axis(1, cex.axis = 2)
  axis(2, cex.axis = 2)
  p1 + theme(
  ...
  pdf(file = "plots/num_clicks_test.pdf",
      width = 10, height = 10, family = "Times")
  par(mar = c(5, 6, 4, 1) + .1)
  p2 <- plot(predict(model, data_test_fold),
             data_test_fold$num_clicks,
...
survival_prob <- function(model, newdata, filename, var, lvl) {
  ...
  surv_plot$plot <- surv_plot$plot +
    theme(
      # Customize axes text sizes
      axis.text.x = element_text(size = 38, family = "serif"),
    ...
    labs(fill = label_expression, color = label_expression) +
  ...
  ggsave(full_file_path, plot = surv_plot$plot,
